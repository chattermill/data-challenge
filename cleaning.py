...
def generate_buckets(data):
	data['apps_bought_bucket'] = pd.cut(data.app_bought, [0,20,60,500], include_lowest=True,labels = ['<20','20-60','>60'])
...
def clean_date(data):
	...
	dates_clean = [pd.to_datetime(date,yearfirst=True) for date in dates]
